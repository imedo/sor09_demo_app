...
module Engines::RailsExtensions::Dependencies
  ...
  def require_or_load_with_engine_additions(file_name, const_path=nil)
    ...
    if file_name =~ /^(.*app\/([a-z_]+)s\/)(.*?)(\.rb)?$/
      file_type = $2
      base_name = $3

      Engines.locations_for(file_type, base_name).each do |file|
        file_loaded = true if require_or_load_without_engine_additions(file, const_path)
      end
