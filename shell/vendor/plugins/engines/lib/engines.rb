...
module Engines
  ...
  # Flag to switch off plugin asset dir
  # 
  mattr_accessor :use_plugin_asset_directory
  self.use_plugin_asset_directory = true
  ...
  self.code_mixing_file_types = %w(controller helper model section)
  ...
  class << self
    ...
    def plugins_with_app_dir
      @plugins_with_app_dir ||= Engines.plugins.select { |plugin| File.directory?(File.expand_path(File.join(plugin.directory, 'app'))) }
    end
    
    def locations_for(file_type, base_name)
      key = "#{file_type}s/#{base_name}"
      @file_locations ||= begin
        locations = {}
        Engines.plugins_with_app_dir.each do |plugin|
          Dir.glob("#{plugin.directory}/app/**/*.rb").each do |file|
            file =~ /^(.*app\/([a-z_]+)s\/)(.*?)(\.rb)?$/
            type = $2
            name = $3
            locations["#{type}s/#{name}"] ||= []
            locations["#{type}s/#{name}"] << file
          end
        end
        locations
      end
